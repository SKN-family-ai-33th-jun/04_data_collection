...
from dataclasses import dataclass       # 단순 값 반환 기능으로서 동작하게끔 해줌.
...
class NaverApiSettings:
    ...
    client_id: str          # dataclass에 의해 생성자를 짧게 작성할 수 있게 됨
    client_secret: str      # <-- 생성자임
...
@dataclass(frozen=True)         # @dataclass: 데이터 타입 형태로 많이 쓰이는 형탸
